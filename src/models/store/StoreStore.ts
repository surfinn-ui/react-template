...
export const StoreStore = types
  ...
  .actions((self) => {
    ...
    const getOrderById = flow(function* (orderId: number) {
      ...
      self.pending();
      const result = yield storeApi.getOrderById(orderId);
      if (result.kind === 'ok') {
        const data = result.data as IOrderModel;
        // TODO - TRANSLATE RESPONSE TO STORE

        // --------------------------------------------------------------------
        console.log('getOrderById()', JSON.stringify(data, null, 2));
        throw new Error('TODO - TRANSLATE RESULT TO STORE getOrderById()');
        // --------------------------------------------------------------------
    ...
    const deleteOrder = flow(function* (orderId: number) {
      if (self.isPending) return;
      self.pending();
      const result = yield storeApi.deleteOrder(orderId);
      if (result.kind === 'ok') {
        ...
        throw new Error('TODO - TRANSLATE RESULT TO STORE deleteOrder()');
        // --------------------------------------------------------------------
    ...
    const placeOrder = flow(function* (payload: any) {
      if (self.isPending) return;
      self.pending();
      const result = yield storeApi.placeOrder(payload);
      if (result.kind === 'ok') {
        const data = result.data as IOrderModel;
        // TODO - TRANSLATE RESPONSE TO STORE
        ...
        throw new Error('TODO - TRANSLATE RESULT TO STORE placeOrder()');
        // --------------------------------------------------------------------
    ...
    const getInventory = flow(function* () {
      if (self.isPending) return;
      self.pending();
      const result = yield storeApi.getInventory();
      if (result.kind === 'ok') {
        const data = result.data as any;
        // TODO - TRANSLATE RESPONSE TO STORE
        ...
        throw new Error('TODO - TRANSLATE RESULT TO STORE getInventory()');
        // --------------------------------------------------------------------
