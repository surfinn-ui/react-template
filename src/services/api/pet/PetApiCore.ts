...
export class PetApiCore extends ApiBase {
  ...
  public async uploadFile(
    ...
    if (additionalMetadata) {
      queries.append('additionalMetadata', String(additionalMetadata));
    }
  ...
  public async updatePetWithForm(
    ...
    const queries = new URLSearchParams();
    if (name) {
      queries.append('name', String(name));
    }
    if (status) {
      queries.append('status', String(status));
    }

    return this.update(`/pet/${petId}?${queries.toString()}`);
  }
   ...
   * @returns
   */
  public async deletePet(petId: number, api_key?: string) {
