...
export class PetApiCore extends ApiBase {
   ...
   * @returns
   */
  public async uploadFile( petId: number, payload: string, additionalMetadata?: string) {
    
      const queries = new URLSearchParams();
      if(additionalMetadata) queries.append('additionalMetadata', additionalMetadata); 

    return this.create<IApiResponseModel>(
      `/pet/${petId}/uploadImage?${queries.toString()}`, payload, {"headers":{"Content-Type":"application/octet-stream"}}
    );
  }
   ...
   * @returns
   */
  public async updatePetWithForm( petId: number, name?: string, status?: string) {
...
if(status) queries.append('status', status); 
  ...
  public async findPetsByTags( tags?: string[]) {
    ...
    return this.search<IPetModel>(
      `/pet/findByTags`, queries
    );
  }
